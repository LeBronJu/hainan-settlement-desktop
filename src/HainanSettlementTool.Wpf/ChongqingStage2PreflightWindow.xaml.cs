...
{
    public partial class ChongqingStage2PreflightWindow : Window
    {
        private readonly List<PreflightIssueItemViewModel> _issueRows;

        public ChongqingStage2PreflightWindow(ChongqingStage2PreflightReport report)
        {
            InitializeComponent();
        ...
        public List<ChongqingStage2SummarySubjectDecision> SummarySubjectDecisions { get; private set; } =
            new List<ChongqingStage2SummarySubjectDecision>();

        private static List<PreflightIssueGroupViewModel> BuildIssueGroups(ChongqingStage2PreflightReport report)
        {
        ...
        private static PreflightIssueItemViewModel BuildIssueItem(ChongqingStage2CheckIssue issue)
        {
            IEnumerable<string> paymentParties = issue.AvailablePaymentParties.Count > 0
                ? issue.AvailablePaymentParties.AsEnumerable()
                : ChongqingStage2PaymentParties.Supported.AsEnumerable();
        ...
        private static string BuildFileText(ChongqingStage2CheckIssue issue)
        {
            ...
            return "模板文件：" + Path.GetFileName(issue.TemplateFile);
        }

        private static string BuildHandlingText(ChongqingStage2CheckIssue issue)
        {
            if (string.IsNullOrWhiteSpace(issue.Suggestion))
            {
                return "处理方式：请人工确认后决定是否继续。";
            }

            return "处理方式：" + issue.Suggestion;
        }

        private static string BuildContextText(ChongqingStage2CheckIssue issue)
        {
        ...
        private void Ok_Click(object sender, RoutedEventArgs e)
        {
            var missing = _issueRows
                ...
                .Select(row => row.SettlementKind + " " + row.Entity)
                .ToList();
            if (missing.Count > 0)
            {
            ...
            SummarySubjectDecisions = _issueRows
                ...
                .Select(row => new ChongqingStage2SummarySubjectDecision
                {
