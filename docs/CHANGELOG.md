...
- 海南 6 月新增主体实机预检反馈修复：同一主体的模板、支付方和默认资料不再分成三个密集问题组，改为一张主体处理卡；完整路径折叠到技术详情。新主体有多份同类型分表可借用时，由操作员显式选一份后继续；零候选、多份同名精确历史模板仍阻断。
...
- 收口验证仅使用合成 workbook：Core 80/80、Excel 121/121（海南专项 53/53、重庆专项 39/39）、WPF 展示 2/2 通过；Debug/Release 解决方案构建、WPF 构建和独立静态审查通过。未读取真实业务工作簿。
