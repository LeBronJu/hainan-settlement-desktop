...
- 收口验证仅使用合成 workbook：Core 72/72、Excel 121/121（海南专项 53/53、重庆专项 39/39）通过；Debug/Release 解决方案构建、WPF 构建、构建可移植性检查和独立静态审查通过。未读取真实业务工作簿。
- 本地 Win10/11 测试包已生成并通过目录/ZIP 内容一致性、数据文件排除和隐藏启动 smoke；测试包、正式包、Win7/8 历史包和本地回测分目录保存。未创建正式 tag 或发布新版本。
