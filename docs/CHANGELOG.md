...
- `v1.4.0` 正式发布：开放广东阶段一“整理本月电量”和“生成本月台账”，同步输出八列电量表、待整理台账、HTML 检查报告和 JSON 记录；无当月电量客户四项电量写数值 `0` 并保留原峰平谷系数。海南、重庆已有目标月份也统一先清空全体客户当月电量再导入，客户名称键增加 NFKC 与不可见空白归一化。
...
- 广东阶段一从只读研究进入开发实现：新增独立 source reader、八列清洗表、编码聚合、首个有效峰平谷系数、32 列月份块、目标月全量清空、系数保留/覆盖、新客户安全追加、JSON 报告和 WPF 两种阶段一入口；`市场联动价格`、`零售合同模式` 和人工辅助 sheet 均不参与阶段一。该改动位于开发分支，尚未合并、打 tag 或正式发布。
- 三省阶段一清理一致性：海南、重庆目标月份原本已存在时也先清空全体台账客户当月电量再写入；共享客户名称键增加 NFKC、不可见空白/零宽字符归一化；海南新增客户行只继承样式与公式，不再继承非公式业务常量。
- 广东阶段一开发分支合成回归：Release Core 87/87、Excel 136/136、WPF 14/14 通过，完整解决方案 Debug/Release 构建 0 警告、0 错误；文档守护、构建可移植性和 diff 检查通过，未读取真实 workbook 或 `outputs/`。随后生成 Win10/11 WPF 本地测试包 `RetailPowerSettlementTool-Win10-11-Release-20260724-154432-647.zip`，包内 20 个文件通过目录/ZIP 逐文件校验，SHA-256 为 `0AB99AE161DC6C72C9B0168EC54106E7EF307B04C9C3F309177B65D2045EBF79`；该包现已被首轮实机反馈修复取代。
- 广东阶段一首轮实机反馈修复：生成前确认改为数字摘要、按类型归纳的重点检查和默认折叠完整明细；按钮、进度步骤与完成提示改为业务语言；无电量客户四项电量由空白改为数值 `0` 并保留系数；“生成本月台账”同步输出八列电量表；两种入口新增自包含 HTML 检查报告并在首页说明下一步。后续按用户确认将标题改为“预检报告”，并将系数提示改为“多计量点峰平谷系数不同客户 / 不影响代理费结算，但建议检查台账”。最新测试包 `RetailPowerSettlementTool-Win10-11-Release-20260724-163923-947.zip` 含 20 个文件，通过目录/ZIP 一致性、业务数据排除和五秒隐藏启动检查，SHA-256 为 `A16435F140D64EF39813EA668308E9CAA7D14E3BC0C3D156C9D9F8EEC8353F84`；此前 `154432-647` 和 `162850-177` 包均已过期。
