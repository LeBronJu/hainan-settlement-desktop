...
- 日志展示
- 进度、状态和结果展示控制器
- 完成页按本次输出能力显示“查看检查报告”，只打开各省本次生成的离线 HTML
- 日志展示和保存入口控制器
- 现代弹窗入口控制器
- 文件/文件夹选择入口控制器
- 输入状态、路径持久化和阶段 options 构造控制器
- 海南阶段一 workflow 编排控制器
- 广东/重庆共享的多省份阶段一 workflow 编排控制器
- 阶段二 workflow 编排控制器
...
- `GuangdongStage1SourceReader`：只读取官方 `零售结算明细`，按表头投影并用 `decimal` 校验/聚合同编码多计量点电量，按来源顺序选择首个有效完整系数
- `GuangdongPowerCleanGenerator`：生成广东八列电量处理表，并允许台账组合入口复用同一次来源读取结果
- `GuangdongLedgerStage1Updater`：按编码检查/写入广东台账，维护 2026 广东月份块、全量初始化目标月电量、系数保留/覆盖、新客户安全追加和写后守恒复核
- `GuangdongStage1ReportWriter`：为只整理电量和生成台账两种入口输出 JSON 完整记录及自包含 HTML 检查报告；HTML 首页突出下一步、重点检查和自动处理结果
- `GuangdongProvinceStage1Adapter`：组合广东 source reader、clean generator 和 ledger updater
...
`ProvinceStage1Service` 只承担多省份阶段一输入生命周期，具体能力由 gateway 的 adapter 注册决定。WPF 使用 `MainWindowProvinceStage1WorkflowController` 编排广东/重庆，省份 profile 提供可见性和文案；海南成熟阶段一继续走自己的 controller，没有在本次扩展中强行迁移。
...
- `HainanSettlementTool.Core.Tests`：覆盖阶段2金额规则、台账/分表差异问题、文件访问保护、共享阶段 workflow 摘要、阶段二预检取消/继续流程、海南员工电量奖励汇总校验、Unicode 客户键，以及广东/重庆阶段一共享服务摘要。
- `HainanSettlementTool.Excel.Tests`：用合成 workbook / `.csv` / `.xlsx` 覆盖原始明细读取、户号冲突处理、三省阶段一目标月全量清空、海南新增行模板安全、输出副本保护、阶段2分表新增客户、汇总表新增主体、模板页脚保护、海南员工电量奖励 workbook 读写、重庆阶段一/阶段二，以及广东阶段一官方 sheet 排除、编码聚合、系数、32 列月份块、新客户和重复运行。
- `HainanSettlementTool.Wpf.Tests`：覆盖广东阶段一 profile、阶段一大量逐户提示按类型聚合/客户预览限制/写零说明、阶段二同主体预检卡聚合、不同费用类型隔离、模板选项紧凑显示、完整路径折叠，以及模板候选 5 个分批、轮内不重复、重新打乱、选择保持和全量关键词 AND 搜索。
...
- 写入当月电量
- 无论目标月份已存在还是从上月复制，都先清空全体存量客户的目标月总量、尖、峰、平、谷，再写本月来源
- 只清洗电量数据并输出 `零售侧用户电量数据处理表.xlsx`
- 新增客户行继承样式和公式但清空非公式业务常量，再写名称和安全字段
- 在原始明细户号唯一明确时补户号
- 输出待人工补齐项
...
- 目标月份区块已存在时同样先清空所有台账客户的目标月五项电量，再按本月来源写入；台账独有客户不保留旧电量
...
广东阶段一已接入 Core、Excel 和 Win10/11 WPF，并随 `v1.4.0` 正式发布；界面提供“整理本月电量”和“生成本月台账”两种入口。

输入：

- 官方 `.xlsx` 零售结算明细 workbook；只读取精确名称 `零售结算明细`
- 广东 2026 售电结算台账（更新时必填）
- 结算月份
- 输出文件夹

输出：

- `x月广东零售侧用户电量数据处理表.xlsx`
- `x月广东零售侧用户电量校验报告.json`
- `x月广东零售侧用户电量校验报告.html`
- `x月广东售电结算台账-阶段一更新.xlsx`
- `x月广东阶段一台账更新报告.json`
- `x月广东阶段一台账更新报告.html`

“生成本月台账”会同时生成八列电量表、台账、JSON 和 HTML，不把清洗结果只留在内存中。
...
- adapter 分发；
- WPF profile、进度、日志、结果展示，以及按问题类型聚合的阶段一确认 presentation；逐客户完整明细默认折叠；
- 通用结果合同与输出安全要求；具体 writer 和报告内容仍由广东 Adapter/Module 实现。

广东 Module 独立负责：

- 精确读取 `零售结算明细`，忽略 `市场联动价格`、`零售合同模式` 和任意人工辅助 sheet；
- 按八个必要表头识别字段，不依赖列位置或总列数；
- 按客户编码聚合计量记录及兆瓦时单位；
- `decimal` 电量守恒和 `0.00001 MWh` 容差；
- 按来源顺序采用同编码首个有效完整 `峰_平/谷_平` 系数；
- 广东 2026 月份块的识别、同宽上月复制、清理和写入；3 月以后标准宽度为 32，1/2 月历史宽度单独识别；
- 新客户插行、格式/公式模板和固定业务字段清空；
- 编码身份、名称冲突和同名不同编码预检；
- 广东输出名和报告文案。

安全写入先暂存系数，再清空所有台账数据行的目标月总量、峰、平、谷和两个系数，并把所有存量客户四项电量初始化为数值 `0`。来源客户按编码覆盖写入四项聚合电量和首个有效系数；台账有但当月无电量的客户保持四项 `0` 并恢复原系数。新客户追加到底部，复制最后一个客户行的样式/公式但清空全部非公式常量，只写序号、编码、名称、`2026MM`、电量和有效系数。

现有 `ProvinceStage1CustomerDecision` 是重庆基于名称的本次决定模型。广东 adapter 明确拒绝该模型，不把名称冒充稳定键；新增编码自动追加，同名不同编码只复核、不合并。

当前不负责：

- 读取/映射 `市场联动价格` 或 `零售合同模式`
- 代理费、居间费、退补金额、税务或代理参数计算
- 2027 跨年度月份块
- 覆盖输入 workbook

完整业务证据、确认项和合成测试边界见 `docs/dev-notes/guangdong-stage1-research-2026-07-24.md`。
...
- WPF 省份 profile 的广东阶段二区域只显示三个分表目录；广东阶段一台账和零售结算明细使用独立阶段一区域。
