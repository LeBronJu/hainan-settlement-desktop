...
- `HainanStage2SettlementGenerator`：海南阶段二入口编排和预检问题构建；保持对 Core 的 Analyze/Generate 行为不变
...
- `ChongqingStage2SettlementGenerator`：重庆阶段二入口编排，调用台账读取、分表写入、汇总写入和报告输出组件；按 30 列月度块识别代理费、居间费、退补电费主体，并对新增汇总主体要求支付方选择
...
- `GuangdongStage2WorkbookInspector` / `GuangdongStage2WorkbookWriter` / `GuangdongStage2ReportWriter`：分别负责广东 workbook 预检分类、目标月 sheet 归一化和 JSON/文本报告
...
- 预检生成轻量不可变处理计划；确认后用文件指纹检测变化，不对未变输入重复完整预检。
- 共享输入每轮只读一次。同一 workbook 的建 sheet、写电量、金额和日期等修改集中到一次生成打开和一次保存。
...
- `ClosedXmlSettlementExcelGateway` 已接入重庆阶段二 Excel gateway；`Analyze` 做生成前支付方预检，`Generate` 写出 workbook 和报告。
...
- 按代理/居间/退补和输入相对路径保存的 workbook 副本
- 广东分表月份初始化 JSON 报告
- 广东分表月份初始化文本校验报告
...
- 每个 workbook 独立失败、整批继续，并在报告和 WPF 完成提示中暴露跳过/失败数量。

不负责：

- 广东台账读取、电量写入、客户匹配、代理/居间/退补金额计算或汇总表生成。
- 把文件名、最后一个 sheet 或 `4-2月新增` 等特殊字符串当作月份来源判断。
- 覆盖输入 workbook，或对无法可靠识别的结构和日期做猜测。
...
4. Excel 性能优化先从海南基线和只读 Adapter 开始，再评估重庆；广东完整结算当前暂缓。
