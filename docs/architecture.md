...
当前完成 Core 合同层、Excel Analyze-only 预检切片和 Win10/11 WPF 预检入口。它可以从桌面端执行预检和新增汇总主体支付方选择，但尚未完成 workbook 生成；因此它仍不是可生成结算文件的功能。
...
- Win10/11 WPF 已开放重庆阶段二预检入口：输入台账、代理模板目录、可选居间模板目录、退补模板目录、汇总表模板和输出目录；确认后只执行预检，不调用 `Generate`，不写出分表、退补表或汇总表。

仍未完成：

- `ChongqingStage2SettlementGenerator` 的分表、退补表和汇总表 workbook 写入。
- 重庆阶段二生成完成后的 WPF 生成入口和输出结果展示。
- 真实 5 月回放验证。
