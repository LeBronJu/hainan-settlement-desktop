...
- 预检 Module 由 `Stage2PreflightIssue`、`Stage2PreflightIssueKinds`、`Stage2PreflightPolicy`、`Stage2RelationshipParameterValidator` 和 `Stage2OpaqueText` 组成，只表达稳定 issue code、`Blocker / RequiredDecision / Review / Information` 处置等级、关系参数合同、完整文本比较、是否允许继续和必选决策生命周期。
...
WPF 共用 `Stage2PreflightPresentationAdapter` 和 `Stage2PreflightWindow`，只负责把 Core 事实分组展示、收集支付方选择和控制确认按钮；它不读取 workbook，也不重新解释税率、主体或模板规则。

海南和重庆通过窄 Adapter 投影省份关系事实并消费共享结果；台账列位、月份块、单位、金额公式、模板/sheet/样式、汇总列位、支付方 sheet 和省份例外继续留在各自 Excel Module。不要引入吸收两省全部差异的 `BaseStage2Generator`、弱类型大 options 或共享层省份 switch。详细规则和实施状态见 `docs/dev-notes/stage2-preflight-integrity-2026-07-22.md`。
...
- `HainanStage2LedgerReader` / `HainanStage2TemplateIndex` / `HainanStage2SplitWorkbookWriter` / `HainanStage2SummaryWorkbookWriter` / `HainanStage2ReportWriter`：海南阶段二 Excel 侧内部组件，分别负责台账读取、模板索引、代理/居间分表写入、汇总表写入、JSON/校验报告输出
...
- `ChongqingStage2LedgerReader` / `ChongqingStage2SplitWorkbookWriter` / `ChongqingStage2SummaryWorkbookWriter` / `ChongqingStage2ReportWriter`：重庆阶段二 Excel 侧内部组件，分别负责台账读取、代理/居间/退补分表写入、汇总表和支付方月度 sheet 写入、JSON/校验报告输出
...
- `GuangdongStage2WorkbookInspector` / `GuangdongStage2WorkbookWriter` / `GuangdongStage2ReportWriter`：分别负责广东 workbook 预检分类、目标月 sheet 归一化、未处理原文件保留和自包含 HTML/JSON/TXT 报告
- `IProvinceStage1Adapter`：Excel 层内部的多省份阶段一 adapter seam
- `ChongqingProvinceStage1Adapter`：组合重庆电量清洗和重庆台账更新实现
- `ClosedXmlUtil`：保留跨海南/重庆共享的 ClosedXML 数字读取和列号转换 helper，不再包含海南台账主表识别逻辑
...
当前已完成 Core 合同层、Excel workbook 写入和 Win10/11 WPF 生成入口。它会先执行预检和新增汇总主体支付方选择，确认后生成代理/居间/退补分表、汇总表副本、JSON 报告和校验报告。该能力已通过合成测试、真实 5 月本地回放、授权 3-5 月阶段一+阶段二回测和用户实机核对，当前作为首个可用生产基线进入长期观察。
