...
Core 层只表达“要做什么”，不关心 Excel 文件具体怎么读写。`SettlementWorkflow` 负责阶段一、只清洗和阶段二执行后的共享结果摘要，UI 仍负责确认、进度展示、日志展示和错误弹窗。
...
- `HainanSettlementTool.Core.Tests`：覆盖阶段2金额规则、台账/分表差异问题、文件访问保护和共享阶段 workflow 摘要。
