...
- `src/HainanSettlementTool.Core/Models/HainanStage2DetailSettlementRow.cs`
...
- 用合成 `HainanStage2DetailSettlementRow` 或输入 DTO 写单元测试，不依赖 ClosedXML。
...
- `src/HainanSettlementTool.Excel/HainanRawDetailReader.cs`
- `src/HainanSettlementTool.Excel/HainanCustomerCodeReader.cs`
...
- `HainanRawDetailReader` 和 `HainanCustomerCodeReader` 只做投影，不各自理解文件格式。
...
- 已新增 `Stage2SettlementCalculator`，集中阶段二明细金额计算、金额格式化和共享容差。2026-07-08 命名治理后，海南台账与分表自算差异问题生成已迁到 `HainanStage2AuditIssueFactory`，避免共享计算器泄漏海南报告模型。
...
- 已新增内部 `HainanRawDetailRowReader` Adapter，统一 `.xlsx` / `.xls` / `.csv` 的行读取、列解析和 CSV 编码兜底；`HainanRawDetailReader` 与 `HainanCustomerCodeReader` 只保留投影逻辑。
- `HainanCustomerCodeReader` 现在也能从 `.csv` 原始零售侧明细读取户号，避免“只清洗电量能读 CSV、补户号不能读 CSV”的漂移。
- 审查后补强：同一客户若在原始明细里出现多个不同户号，不自动选择第一条，而是不给出户号映射，让阶段一报告继续提示人工补齐；同一客户重复且户号一致仍可自动补齐。
- 已为 `HainanStage1LedgerUpdater` 补齐合成 workbook 测试，覆盖新增客户插入 footer 前、唯一户号补齐/冲突户号不自动补、输出台账副本不覆盖基础台账。
