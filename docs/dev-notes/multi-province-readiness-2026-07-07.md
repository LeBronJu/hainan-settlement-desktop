...
- `ProvinceStage1Service` 的“是否支持”和通用必填项仍偏重庆专用；广东阶段一研究已经使这项技术债从远期提醒变成正式接入前置条件。
- `MainWindow.xaml.cs` 已拆出重庆阶段一 workflow 编排逻辑；主窗口主要保留事件路由、主题、省份切换和少量壳层协调。
- GitHub 仓库、Win10/11 WPF exe 和发布包已在 `v1.1.0` 收口时中性化；解决方案名、项目目录、根命名空间仍保留 `HainanSettlementTool.*`，不做一次性大迁移。
- 完整 MVVM、持久化客户别名表、跨省通用阶段二抽象仍是 P2。

### 2026-07-24 广东阶段一研究更新

本段只转述另一次经用户明确授权的只读 workbook 研究结论，不改变本架构自查本身没有读取真实 Excel 的事实。

- 广东官方月度来源的可靠身份键是 `电力用户编码`，不能复用重庆的客户名称匹配。
- 交易中心原始下载只有 `零售结算明细`、`市场联动价格`、`零售合同模式` 三个官方输入 sheet；研究用加工副本后附的两个 sheet 是结算员人工制作，程序必须完全排除。
- 广东台账使用省份专属 32 列月份块；月份块识别、清空范围、新客户行和合同映射都必须留在独立广东 Adapter。
- 现有 `IProvinceStage1Adapter` 字典 seam 可以承载广东，但 Core 的重庆支持门禁、WPF 重庆 controller 和通用“户号/多户号”摘要需要先中性化。
- 现有客户决定模型以名称为主。广东如果需要人工匹配，应先增加稳定 source/target key，不能用名称冒充客户编码。
- 完整结构、建议实现顺序和待确认业务项见 `docs/dev-notes/guangdong-stage1-research-2026-07-24.md`。
...
5. 全项目命名规范化已完成当前低风险收口；后续新增省份继续遵循“省份专属 Module 带省份名、真实跨省共享 Module 才用通用名”。对外仓库、WPF exe 和发布包使用中性品牌；保留解决方案名、项目目录和根命名空间，不做高风险大迁移。

### P1：第三个省接入时同步做

1. 为新省份建立独立 Excel Module，不把新省规则塞进重庆或海南类。（广东阶段二月份初始化已遵守；广东阶段一尚待实现）
2. 补齐新省份 synthetic workbook 测试。（广东阶段二月份初始化已完成；广东阶段一尚待实现）
3. 更新 `CONTEXT.md` 的省份业务口径，并更新 `docs/architecture.md` 的阶段边界。（广东阶段二已完成；广东阶段一研究边界已记录）
...
### 2026-07-10 Excel 性能与引擎迁移方向

- 广东第一批已用 600 多个真实 workbook 做用户批量验收，耗时 4 分多钟，当前满足临时需求。
- 下一个性能试点先做海南，再评估重庆；广东完整结算和性能扩展暂缓。
- 性能优化必须保持省份 Module 边界，不能为共享并行管线泄漏海南/重庆/广东列位和模板规则。
- 当前候选终态是 ClosedXML/Open XML 混合 Adapter，不做一次性引擎替换。
- 详细风险、海南试点顺序和验收门槛见 `docs/dev-notes/excel-performance-openxml-research-2026-07-10.md`。
