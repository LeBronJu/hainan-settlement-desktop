...
- `IProvinceStage1ExcelGateway` 这个 seam 是对的，但当前 adapter 分发在 `ClosedXmlStage1ExcelGateway` 里写死重庆。
...
- `ClosedXmlStage1ExcelGateway` 只维护 `Dictionary<ProvinceCode, IProvinceStage1Adapter>` 并转发调用。
...
- 海南命名逐步中性化已进入今日多省份技术债主线。先从内部类名、变量名和 helper 名称切小步改，暂不做项目名/命名空间/发布包名的一次性大重命名。
...
5. 开始海南命名中性化的低风险切片：先改内部多省份概念的类名/变量名/helper 名，不做根项目名、程序集名、命名空间和发布包名的大迁移。
