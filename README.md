...
- Win10/11版（WPF）支持结算省份选择，启动时默认不选省份，需手动选择后再执行。海南显示成熟结算流程；重庆当前开放阶段一的“只清洗电量数据”和“清洗并更新台账”。
...
- 重庆台账更新按 `电力用户名称` 精确匹配，不依赖也不自动补齐 `电力用户编码`；户号只保留在清洗明细和报告中用于追溯。匹配异常、多户号、疑似名称别名和已有电量差异会在写入前弹窗确认；用户可在弹窗里把本次未匹配的清洗客户手动匹配到台账客户，该选择只影响当前生成副本，并写入 JSON 更新报告。
