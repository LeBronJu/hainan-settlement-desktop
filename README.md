...
- 重庆阶段2：人工整理后的重庆台账 -> 代理/居间/退补分表、汇总表副本、JSON 报告、阶段二校验报告。
...
当前解决方案已编译通过。2026-07-22 阶段二完整性与新增主体预检交互分支仅使用合成 workbook 做开发验证：Core 80/80、Excel 121/121、WPF 展示 2/2 通过，完整解决方案 Debug/Release 构建通过。当前分支已生成 Win10/11 本地测试包供实机验证；`v1.1.0` 仍是最新正式版，本次没有制作或发布新正式版。
...
- 海南和重庆阶段二使用同一套预检处置语义和 WPF 确认窗口：同一 `(费用类型, 主体)` 的选择和复核项集中到一张主体卡，完整路径默认折叠。新主体或缺失支付方显式选择清能/清辉；海南新主体有多份可借用的同类型分表时，同卡显式选择一份模板，不再因“候选多于一份”直接阻断。收款人按完整单元格文本继承，不拆分多人姓名；不生成付款路由 JSON。正式结果先在隔离区完整生成和核验，再整批发布；当前不提供部分汇总表。
- 重庆阶段二读取重庆台账、代理/居间/退补模板目录和汇总表，生成代理/居间/退补分表、汇总表副本、JSON 报告和校验报告。首版已通过合成测试、授权真实 5 月本地回放 smoke 和 3-5 月阶段一+阶段二回测；退补额外扣减、历史人工抵扣等特殊项仍通过校验报告和完成后弹窗提示人工复核。
