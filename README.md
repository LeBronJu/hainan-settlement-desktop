...
- 海南和重庆阶段二使用同一套预检处置语义和 WPF 确认窗口：阻断资料错误、要求新主体或缺失支付方显式选择清能/清辉，并把新增客户、多负责人、模板借用和合法字段变化列为复核。收款人按完整单元格文本继承，不拆分多人姓名；不生成付款路由 JSON。正式结果先在隔离区完整生成和核验，再整批发布；当前不提供部分汇总表。
