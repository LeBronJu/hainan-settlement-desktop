...
- 重庆台账更新按 `电力用户名称` 精确匹配，不依赖也不自动补齐 `电力用户编码`；户号只保留在清洗明细和报告中用于追溯。匹配异常、多户号、疑似名称别名和已有电量差异会在写入前弹窗确认；用户可在弹窗里把本次未匹配的电量表客户手动匹配到台账客户，并且每个待判断客户都必须显式选择匹配目标或选择本月不写入。该选择只影响当前生成副本，并写入 JSON 更新报告。
