...
- 最新正式版本：`v1.4.0`
- Release 页面：`https://github.com/LeBronJu/retail-power-settlement-desktop/releases/tag/v1.4.0`
- 当前主线：`main`
- `v1.4.0` 正式开放广东阶段一：整理交易中心零售结算明细、生成八列电量表并写入本月台账；同时统一海南、重庆已有目标月份的全客户电量清理规则。
- 当前正式包：
  - `RetailPowerSettlementTool-Win10-11-v1.4.0.zip`
...
- 广东阶段一：可“整理本月电量”生成八列电量表，或“生成本月台账”同时生成八列电量表和待整理台账；两种入口均输出自包含 HTML 检查报告和 JSON 完整记录。生成前界面按类型归纳重点，完整逐户明细默认折叠。
- 广东分表月份初始化：批量处理代理/居间/退补 workbook，创建或整理标准目标月份 sheet，清空 C-F 电量并顺延日期；正常输出确保目标月份 sheet 可见、默认打开且只有该 sheet 被选中。输出同时包含 HTML/JSON/TXT 检查报告；日期或结构异常的原文件会原样保留到 `【未处理-需人工复核】`，界面明确显示部分完成，不自动纠正业务日期。当前不计算广东结算金额。
- 广东分表月份初始化及后续安全加固包已通过用户实测；用户运行 600 多个真实 workbook 总耗时 4 分多钟，当前性能满足临时需求。2026-07-22 用户完成已发布全省份功能实机测试，未发现新的问题。广东完整阶段二金额结算和海南/重庆性能优化仍暂缓。
- 界面顶部“公共设置”里的结算月份和结果输出文件夹由阶段1、阶段2共用；员工电量奖励使用同一个结果输出文件夹，并在模块内选择开始/结束月份。
- Win10/11版（WPF）支持结算省份选择，启动时默认不选省份，需手动选择后再执行。海南显示成熟结算流程；重庆开放阶段一和阶段二生成；广东同时显示阶段一电量/台账入口及代理/居间/退补分表月份初始化入口。切换省份时会清空上一省份专用输入，避免跨省误用路径。
...
2026-07-24 广东阶段一经过三轮本地测试包反馈后进入 `v1.4.0` 正式版。Release 回归为 Core 87/87、Excel 136/136、WPF 14/14；正式包继续只包含维护中的 Win10/11 WPF 入口，不包含真实 workbook、结算输出或客户资料。完整发布验证与 SHA-256 记录见 `HANDOFF.md`。
...
```powershell
.\scripts\package_wpf_release.ps1 -ReleaseTag v1.4.0
```
...
- 海南和重庆阶段一无论目标月份是新复制还是已经存在，都先清空全体台账客户的当月电量列再导入；台账本月无来源电量的客户不会保留旧值。
- 广东阶段一只读取精确名称 `零售结算明细`，按 `用户编号` 匹配台账 `电力用户编码`；同编码多计量点电量相加，`峰_平/谷_平` 按原始顺序采用首个有效完整系数。电量用 `decimal` 和 `0.00001 MWh` 容差校验。目标月先清空四项电量和两个系数；台账本月无电量客户四项电量写数值 `0`，原系数保留。新编码追加到底部，只继承样式/公式并写安全字段，其余合同、税务、代理资料人工补齐。
- 海南和重庆阶段二使用同一套预检处置语义和 WPF 确认窗口：同一 `(费用类型, 主体)` 的选择和复核项集中到一张主体卡，完整路径默认折叠。新主体或缺失支付方显式选择清能/清辉；两省新主体有多份可借用的同类型分表时，同卡显式选择一份模板，不再因“候选多于一份”直接阻断。模板候选较多时每批随机展示最多 5 份，同一轮不重复；可查看上一批/换一批，全部看完后重新打乱，也可按主体、负责人和文件名搜索本次完整候选集。换批不会清除已选模板，最终选择仍受预检签名和输入指纹保护。海南新增主体借用其它主体模板时不带入来源人的历史 sheet 或批注；存量主体自己的批注继续保留。生成的分表确保本次结算月份 sheet 可见、默认打开且只有该 sheet 处于选中状态。收款人按完整单元格文本继承，不拆分多人姓名；不生成付款路由 JSON。正式结果和 HTML/JSON/TXT 报告先在隔离区完整生成和核验，再整批发布；当前不提供部分汇总表。
