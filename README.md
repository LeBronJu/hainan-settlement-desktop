...
- 海南和重庆阶段二使用同一套预检处置语义和 WPF 确认窗口：同一 `(费用类型, 主体)` 的选择和复核项集中到一张主体卡，完整路径默认折叠。新主体或缺失支付方显式选择清能/清辉；海南新主体有多份可借用的同类型分表时，同卡显式选择一份模板，不再因“候选多于一份”直接阻断。新增主体借用其它主体模板时不带入来源人的历史 sheet 或批注；存量主体自己的批注继续保留。生成的分表确保本次结算月份 sheet 可见、默认打开且只有该 sheet 处于选中状态。收款人按完整单元格文本继承，不拆分多人姓名；不生成付款路由 JSON。正式结果和 HTML/JSON/TXT 报告先在隔离区完整生成和核验，再整批发布；当前不提供部分汇总表。
