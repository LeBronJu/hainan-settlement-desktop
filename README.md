...
- 最新正式版本：`v1.3.0`
- Release 页面：`https://github.com/LeBronJu/retail-power-settlement-desktop/releases/tag/v1.3.0`
- 当前主线：`main`
- `v1.3.0` 为海南、重庆新增可扩展的阶段二分表模板浏览与搜索，重庆多候选模板由阻断改为人工必选；海南、重庆分表和广东月份初始化结果打开时默认进入本次结算月份。
- 当前正式包：
  - `RetailPowerSettlementTool-Win10-11-v1.3.0.zip`
...
当前解决方案已编译通过。2026-07-24 阶段二模板浏览器、重庆模板决策闭环和目标月份默认打开行为仅使用合成 workbook 做自动化验证：Core 83/83、Excel 129/129、WPF 11/11 通过，完整解决方案 Debug 测试和 Release 构建通过；构建可移植性、文档守护与发布包数据文件排除检查均通过。此前 `v1.2.0` 发布前的全省份实机测试未发现问题；本轮 `v1.3.0` 改动在正式发布前也已提供独立测试包。
...
```powershell
.\scripts\package_wpf_release.ps1 -ReleaseTag v1.3.0
```
